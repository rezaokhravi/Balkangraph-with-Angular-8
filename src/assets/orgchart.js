...
OrgChart.prototype.load = function (t) {
  ...
  for (c = 0; c < s.length; c++) {
    ...
    for (w = _.children.length - 1; 0 <= w; w--) {
      (F = _.children[w]).isAssistant ? (F.parent = null, _.children.splice(w, 1), _.childrenIds.splice(w, 1), A.splice(0, 0, F.id)) : (F.pid = k.id, (F.parent = k).children.push(F), k.childrenIds.push(F.id), _.children.splice(w, 1))
    }
    if (A.length % 2) {
      ...
      OrgChart.templates[I] = {
        ...
        nodeMenuButton: "",
        size: N
      }, (W = new OrgChart.node(_.id + "_transperant", S.pid, null, [], I)).isAssistant = !0, n[W.id] = W, A.splice(A.length - 1, 0, W.id)
    }
    var E = 1;
    for (w = A.length - 1; 0 <= w; w--) {
      if (w % 2 && w != A.length - 1)(J = new OrgChart.node(_.id + "_split_" + E, null, null, [], "split")).isAssistant = !0, n[J.id] = J, A.splice(w, 0, J.id), E++;
    ...
    for (w = 0; w < A.length; w += 3) {
      ...
      var L = n[A[w]],
        B = n[A[w + 1]],
        T = n[A[w + 2]];
      L.parent = M, L.pid = M.id, B.parent = M, B.pid = M.id, T.parent = M, T.pid = M.id, M.children.push(L), M.children.push(B), M.children.push(T), M.childrenIds.push(L.id), M.childrenIds.push(B.id), M.childrenIds.push(T.id);
      for (var P = M;
        "split" == P.templateName && (P = P.parent);) P.childrenIds.push(L.id), P.childrenIds.push(B.id), P.childrenIds.push(T.id);
      (x = i[B.id]) && (n[B.id].state = x.state, n[B.id].state == OrgChart.COLLAPSE && O.push(B.id)), OrgChart.server._initStates(B, n, e, t, r, O)
    }
  }
  if (e == OrgChart.action.centerNode) {
    ...
    for (var z = H.id; null != _;) {
      if (-1 != (D = O.indexOf(_.id)) && O.splice(D, 1), _.state = OrgChart.EXPAND, r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
        for (c = 0; c < _.childrenIds.length; c++) {
          ...
          if (!OrgChart.server._isSplit(F) && !F.isAssistant && F.id != z) - 1 == (D = O.indexOf(_.childrenIds[c])) && O.push(_.childrenIds[c]), F.state = OrgChart.COLLAPSE
        }
    ...
    if (r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
      for (c = 0; c < H.childrenIds.length; c++) {
        if (-1 != (D = O.indexOf(H.childrenIds[c])) && O.splice(D, 1), (F = n[H.childrenIds[c]]).state = OrgChart.EXPAND, !F.isAssistant && !OrgChart.server._isSplit(F))
          for (w = 0; w < F.childrenIds.length; w++) {
            var D, G = n[F.childrenIds[w]]; - 1 == (D = O.indexOf(F.childrenIds[w])) && O.push(F.childrenIds[w]), G.state = OrgChart.COLLAPSE
          }
      }
  }
          ...
          if (-1 == _.collapsedChildrenIds.indexOf(y.id) && _.collapsedChildrenIds.push(y.id), _.children.splice(w, 1), "split" == y.templateName || "split" == _.templateName) {
            if (M = y.parent) {
              ...
              for (var R = 0; R < M.childrenIds.length; R++) {
                var X = M.childrenIds[R]; - 1 == M.collapsedChildrenIds.indexOf(X) && M.collapsedChildrenIds.push(X)
              }
            }
          } else y.parent = null
      }
        ...
        if (_ = y.parent)
          for (w = _.children.length - 1; 0 <= w; w--) {
            if ((F = _.children[w]) != y && 0 == F.childrenIds.length && !F.isAssistant && "split" != F.templateName) {
              F.parent = y, F.pid = y.id, F.layout = OrgChart.mixed, _.children.splice(w, 1);
              var q = y.children.length;
              y.children[q] = F, y.layout = OrgChart.mixed
            }
          }
    }
  if (t.layout == OrgChart.tree || t.layout == OrgChart.treeRightOffset || t.layout == OrgChart.treeLeftOffset) {
    var Y = [];
    for (c = 0; c < o.length; c++) {
      if (0 == (y = n[o[c]]).childrenIds.length && !y.isAssistant && "split" != y.templateName)(_ = y.parent) && -1 == Y.indexOf(_.id) && Y.push(_.id)
    }
    for (c = 0; c < Y.length; c++) {
      _ = n[Y[c]], k = new OrgChart.node(_.id + "_split_0", _.id, null, [], "split");
      ...
      for (w = _.children.length - 1; 0 <= w; w--) {
        if (0 == (F = _.children[w]).childrenIds.length && !F.isAssistant && "split" != F.templateName && (F.parent && "split" != F.parent.templateName || F.parent.isAssistant)) {
          if ((F = _.children[w]).parent = null, F.layout = OrgChart.tree, _.children.splice(w, 1), _.childrenIds.splice(w, 1), t.layout == OrgChart.treeRightOffset && V.splice(0, 0, F.id), t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
            var W;
            ...
            OrgChart.templates[I] = {
              ...
              nodeMenuButton: "",
              size: [0, 0]
            }, (W = new OrgChart.node(F.id + "_transperant", null, null, [], I)).layout = OrgChart.tree, n[W.id] = W, V.splice(0, 0, W.id)
          }
          t.layout != OrgChart.treeRightOffset && V.splice(0, 0, F.id)
        }
      }
      for (E = 1, w = V.length - 1; 0 <= w; w--) {
        var J;
        if (w % 2 && w != V.length - 1)(J = new OrgChart.node(_.id + "_split_" + E, null, null, [], "split")).layout = OrgChart.tree, n[J.id] = J, V.splice(w, 0, J.id), E++;
      ...
      for (w = 0; w < V.length; w += 3) {
        ...
        0 != w && (M = n[V[w - 3]]), 0 == w || B || (M = n[V[w - 2]]), L.parent = M, L.pid = M.id, M.children.push(L), _.childrenIds.push(L.id), B && (0 != w && (M = n[V[w - 2]]), B.parent = M, B.pid = M.id, M.children.push(B), _.childrenIds.push(B.id)), T && (0 != w && (M = n[V[w - 1]]), T.parent = M, T.pid = M.id, M.children.push(T), _.childrenIds.push(T.id))
      }
    }
  }
  return {
  ...
  for (var S in g) {
    ...
    if (OrgChart.server._isVisible(N, i, s, d)) {
      ...
      null != I && null != I.x && null != I.y && (I.x == N.x && I.y == N.y || (m[0].push(N.id), m[1].push({
